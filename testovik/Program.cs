...
namespace testovik
{
    class Program
    {
        ...
        public static async void load(UserStor _userstor)
        {
            using (StreamReader sr = new StreamReader("Test.txt", System.Text.Encoding.Default))
            {
                string line;
                //if (sr.ReadLineAsync()!=null)
                while ((line = await sr.ReadLineAsync()) != null) {_userstor.loadobj(sr);}
                sr.Close();
                //else _userstor.loadobj(sr);
        ...
        public static async void OnMessageHandler(object sender, MessageEventArgs e)
        {
            ...
            if (msg.Text!=null)
            {
                ...
                if (msg.Text=="/add") {Addu.execute(msg,client,userstor);}
                else if (msg.Text=="/choose") {commands[0].execute(msg,client,userstor); if (Addu.active==true) Addu.stopex(msg,client);}
                else if (msg.Text=="/del") {commands[2].execute(msg,client,userstor); if (Addu.active==true) Addu.stopex(msg,client);}
                ...
                else if (msg.Text.StartsWith("@") && !msg.Text.StartsWith("@ "))
                {
                    ...
                    {
                        char Mychar='@';
                        tguser=new TgUser(msg.Text.TrimStart(Mychar));
                        userstor.addu(tguser);
                        await client.SendTextMessageAsync(msg.Chat.Id,"@"+userstor.getu(userstor.count-1).name+" is added. Enter username",replyMarkup:GetButtons());
                        Addu.stopex(msg, client);
                        savestor(userstor);
                    }
                }
                else if (msg.Text=="/show") {commands[1].execute(msg,client,userstor); if (Addu.active==true) Addu.stopex(msg,client);}
                ...
                {
                    await client.SendTextMessageAsync(msg.Chat.Id,"start",replyMarkup:GetButtons());
                    if (Addu.active==true) Addu.stopex(msg,client); load(userstor);
                }
            }
        }
